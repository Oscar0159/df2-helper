...
const SelectHoverContext = createContext<SelectHoverContextValue | null>(null);
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectHoverContext.Provider
        value={{ hoveredItem, setHoveredItem, hoverLayoutId }}
      >
...
function SelectItem({
  ...
  const hoverContext = useContext(SelectHoverContext);
  const itemValue = props.value;
  const isHovered = hoverContext?.hoveredItem === itemValue;
  ...
  return (
    <SelectPrimitive.Item
      ...
      onPointerMove={(event) => {
        hoverContext?.setHoveredItem(itemValue);
        onPointerMove?.(event);
      }}
      onPointerLeave={(event) => {
        if (hoverContext?.hoveredItem === itemValue) {
          hoverContext.setHoveredItem(null);
        }
        onPointerLeave?.(event);
      }}
      {...props}
    >
      {isHovered && hoverContext ? (
        <motion.span
          layoutId={hoverContext.hoverLayoutId}
          className="absolute inset-0 rounded-md bg-accent/70"
