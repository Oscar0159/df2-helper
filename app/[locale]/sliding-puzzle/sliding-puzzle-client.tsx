...
function getSuggestionBadgeClass(cols: number) {
  return cols === 2
    ? "top-1.5 right-1.5 size-5.5 text-[10px]"
    : "top-2 right-2 size-6 text-[11px]";
}
...
export function SlidingPuzzleClient() {
  ...
  const [imageId, setImageId] =
    useState<(typeof imageOptions)[number]["id"]>(defaultImageId);
  ...
  const [playbackSpeed, setPlaybackSpeed] =
    useState<(typeof playbackSpeedOptions)[number]>(1);
  ...
  const solution = useMemo(
    () => getSolutionDetails(deferredBoard),
    [deferredBoard],
  );
  const solved = useMemo(() => isSolved(board), [board]);
  const currentImage =
    imageOptions.find((option) => option.id === imageId) ?? imageOptions[0];
    ...
    return board.flatMap((row, rowIndex) =>
      row.map((tile, colIndex) => {
        ...
        return {
          ...
          isBlank: tile === 0,
          isMovable:
            tile === 0 ? false : canSlideTile(board, rowIndex, colIndex),
        };
      }),
    );
  ...
  const tileMap = useMemo(
    () =>
      new Map(
        cells
          .filter((cell) => cell.tileId)
          .map((cell) => [cell.tileId as string, cell]),
      ),
    [cells],
  );
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
      <section className="tool-panel order-2 space-y-5 xl:col-start-1 xl:row-start-1">
        ...
        <div className="tool-subpanel space-y-4">
          ...
          >
            <div className="rounded-[20px] bg-muted/35 p-3 sm:p-4">
      ...
      <div className="contents xl:col-start-2 xl:row-start-1 xl:grid xl:gap-6">
        <section className="tool-panel order-1 space-y-4 xl:order-1">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <p className="hidden text-muted-foreground text-sm xl:block">
                {formatImageLabel(currentImage.id)}
              </p>
            </div>
            ...
            <div className="hidden xl:block">
              <ImagePickerGrid imageId={imageId} onSelect={setImageId} />
            </div>
          </div>
        </section>

        <section className="tool-panel order-3 space-y-4 xl:order-2">
              ...
              <div className="tool-subpanel space-y-4">
                <div className="space-y-2 sm:flex sm:items-center sm:justify-between sm:gap-3 sm:space-y-0">
                  <span className="tool-section-label">
                    {t("playbackSpeedTitle")}
                  </span>
                  <ButtonGroup className="relative isolate flex w-full flex-wrap overflow-hidden rounded-2xl border border-border/60 bg-background/75 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.24)] backdrop-blur-sm sm:ml-auto sm:w-auto sm:rounded-full">
                      ...
                      >
                        {playbackSpeed === speed ? (
                          <motion.span
                            ...
                            className="absolute inset-0 rounded-xl border border-border/70 bg-card shadow-[0_1px_2px_rgba(24,24,27,0.06),0_6px_18px_rgba(24,24,27,0.08)] sm:rounded-full"
                            transition={{
                              type: "spring",
                              stiffness: 380,
                              damping: 30,
        ...
        return (
          ...
          >
            <motion.div
              ...
              className={cn(
                "relative aspect-square overflow-hidden rounded-[16px] bg-muted ring-1 transition-all duration-200",
                active
                  ? "shadow-[0_10px_28px_rgba(24,24,27,0.14)] ring-2 ring-primary"
                  : "ring-border/50 group-hover:ring-foreground/20",
              )}
            >
              <Image
                src={option.src}
                ...
                className={cn(
                  "object-cover transition-all duration-200 group-hover:scale-[1.04]",
              ...
              <AnimatePresence>
                {active ? (
                  <motion.span
                    ...
                    className="absolute top-2 right-2 flex size-6 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm"
                  >
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        radiusClass,
        isBlank
          ? "bg-background/75 ring-1 ring-border/60 ring-dashed"
          : "bg-background/35 ring-1 ring-border/40",
        highlightBlank &&
          "shadow-[0_0_0_4px_rgba(24,24,27,0.06)] ring-2 ring-primary/70",
        isOver && "ring-2 ring-primary/60",
      )}
    >
      <div className="aspect-square">
        {isBlank ? (
          <div className="absolute inset-0 grid place-items-center text-center text-muted-foreground text-xs uppercase leading-none tracking-[0.18em]">
            {emptyLabel}
          </div>
        ) : (
          <DraggablePuzzleTile
  ...
  return (
    <motion.button
      ...
      whileHover={
        isDragging
          ? undefined
          : { y: isMovable ? -2 : -1, scale: isMovable ? 1.01 : 1.005 }
      }
      whileTap={{ scale: 0.985 }}
      transition={{ type: "spring", stiffness: 320, damping: 24 }}
      className={cn(
        "relative size-full overflow-hidden bg-card text-left outline-none transition-all duration-200 focus-visible:ring-3 focus-visible:ring-ring/50",
        radiusClass,
        isMovable && "cursor-pointer ring-2 ring-primary/60",
        isMovable && "shadow-[0_10px_24px_rgba(24,24,27,0.08)]",
        !isMovable && "cursor-grab ring-1 ring-border/50",
        !isMovable &&
          "shadow-[0_8px_20px_rgba(24,24,27,0.06)] hover:ring-foreground/20",
      ...
      <AnimatePresence>
        {suggestedIndex ? (
          <motion.span
            ...
            className={cn(
              "absolute flex items-center justify-center rounded-full bg-primary font-semibold text-primary-foreground shadow-sm",
...
function PuzzleTilePreview({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative size-full overflow-hidden bg-muted transition-all duration-200",
        radiusClass,
      ...
      <AnimatePresence>
        {showNumbers ? (
          <motion.span
            ...
            className={cn(
              "absolute rounded-full bg-background/92 font-semibold leading-none shadow-sm",
              numberBadgeClass,
            )}
          >
            {tile}
